...
def add_classes(UIN):
    if(request.method == "POST"):
        ...
        if class_exist:
            ...
            enroll_class(conn, class_name, class_descr, class_type, class_semester, class_year, UIN)
            conn.close()
...
def update_class(Class_ID, UIN):
    ...
    if (request.method == 'POST'):
        ...
        update_enrollment_info(conn, Class_ID, UIN, request.form["class_semester"], request.form["class_year"])
        conn.close()
...
def enroll_class(conn, class_name, class_descr, class_type, semester, year, UIN):
    class_ = get_class_by_name(conn, class_name)
    if class_ is None:
        add_new_class(conn, class_name, class_descr, class_type)
        class_ = get_class_by_name(conn, class_name)
    conn.execute('INSERT INTO Class_Enrollment (UIN, Class_ID, Status, Semester, Year) VALUES (?, ?, "Enrolled", ?, ?)', (UIN, class_['Class_ID'], semester, year))
    conn.commit()
...
def update_enrollment_info(conn, class_id, UIN, class_semester, class_year):
    conn.execute('UPDATE Class_Enrollment SET Semester=?, year=? WHERE Class_ID=?', (class_semester, class_year, class_id))
    conn.commit()
